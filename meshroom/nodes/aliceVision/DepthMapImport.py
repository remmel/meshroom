...
class DepthMapImport(desc.Node):
...
'''
    ...
    def readInputDepth(self, depthPath):
        if depthPath.endswith(".png"):
            ...
            depths, confidences = Utils.readAndroidDepth16(depthPath)
            return depths
...
class Utils:
    ...
    # FIXME remove default value h,w
    def readAndroidDepth16(input, h=180, w=240):
        depths = np.zeros((h, w), np.uint16)
        confidences = np.zeros((h, w), np.ushort)
        with open(input, "rb") as f:
            ba = bytearray(f.read())
            if not len(ba)==w*h*2 : raise RuntimeError("Size of array doesnt watch input size", "len(ba):",len(ba))
